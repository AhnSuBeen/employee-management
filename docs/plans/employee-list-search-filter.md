...
JPQL `@Query`로 대소문자 무관 부분 일치 쿼리를 하나 추가해 세 조건의 AND 결합과
기존 부서→이름 정렬을 동시에 처리한다. 뷰는 `list.html`을 사이드바(검색 폼) + 기존
카드(표) 2단 레이아웃으로 바꾸고, 검색값을 폼에 다시 채워 넣는다.
...
1. `EmployeeRepository`
   - JPQL 검색 쿼리 추가:
     ...
     ```
   - 처음에는 `(:name IS NULL OR ...)` 형태로 빈 조건을 건너뛰려 했으나, 값이 없을 때
     `null` 파라미터를 넘기면 PostgreSQL이 `LOWER(?)`의 인자 타입을 추론하지 못해
     500 에러(`함수가 없음`)가 발생함을 구현 중 확인했다. 대신 빈 값은 `null`이 아니라
     **빈 문자열(`""`)**로 정규화해 넘기고, 쿼리에서는 `IS NULL` 분기를 없앴다 —
     `LIKE '%%'`는 모든 문자열에 매치되므로 "조건 없음"과 동일하게 동작한다.
   - 기존 `findAllByOrderByDepartmentAscNameAsc()`는 검색 조건이 모두 빈 문자열일 때도
     `search("", "", "")`로 동일하게 커버되므로 제거하고 `search`로 통합한다.
2. `EmployeeService`
